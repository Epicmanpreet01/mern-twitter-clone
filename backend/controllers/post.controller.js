...
export const getFollowingPosts = async function(req,res) {
  ...
  try {
    ...
    const followingUserPosts = await Post.find({poster: {$in: followingUsers}}).sort({createdAt: -1}).populate({
      path: 'poster',
      select: '-password'
    }).populate({
      path: 'comments',
    ...
    console.error(`Error occured while fetching following posts: ${error.message}`);
...
export const getUserPosts = async function(req,res) {
  ...
  try {
    ...
    const otherUserPosts = await Post.find({poster: otherUser._id}).sort({createdAt: -1}).populate({
      path: 'poster',
      select: '-password'
    }).populate({
      path: "comments",
...
export const createPost = async function(req,res) {
  ...
  try {
    ...
    if(!text && !img) return res.status(400).json({error: 'Post can only be created if one of or both image and text are provided'});
...
export const commentOnPost = async function(req,res) {
  ...
  try {
    ...
    if(!text && !img) return res.status(400).json({error: 'At least an image or a text in needed to create a comment'});
...
export const deletePost = async function(req,res) {
  ...
  try {
    ...
    if(userId.toString() !== post.poster.toString()) return res.status(401).json({error: 'Unautharized access, only author of the post can delete the post'});
    ...
    if(post.parentId !== null) {
      const parentPost = await Post.findByIdAndUpdate(post.parentId, {$pull: {comments: postId}});
    }
    ...
    console.error(`Error occured in deleting a post: PostId: ${postId}, error: ${error.message}`);
    return res.status(500).json({error: error.message});
  }
}