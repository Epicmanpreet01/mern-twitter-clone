...
export const getNotifications = async function(req,res) {
  ...
  try {
    ...
    if(notifications.length === 0) return res.status(200).json({message: 'No notifications for this user', data:[]});
...
export const deleteNotifications = async function(req,res) {
  ...
  try {
    ...
    if(deletedNotifications.length === 0) return res.status(200).json({message: 'No notifications to delete'});
...
export const deleteNotification = async function(req,res) {
  ...
  try {
    ...
    if(notification.to.toString() !== userId.toString()) return res.status(403).json({error: 'You are not autharized to delete this notification'});

    const deletedNotification = await Notification.findByIdAndDelete(id);
    
    return res.status(200).json({message: 'Notification deleted successfully'});
  } catch (error) {
    console.error(`Error occured while deleting notification with id: ${id} with error message: ${error.message}`);
