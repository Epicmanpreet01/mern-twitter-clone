...
export const followUnfollowUser = async function (req, res) {
  ...
  try {
    if (isFollowing) {
      ...
      return res
        .status(200)
        .json({ message: "User unfollowed successfully", data: otherUser._id });
    } else {
      //Follow the user
      ...
      return res
        .status(200)
        .json({ message: "User followed successfully", data: otherUser._id });
    }
  } catch (error) {
...
export const updateProfile = async function (req, res) {
  let { name, userName, email, currentPassword, newPassword, bio, link } =
    req.body;
  ...
  try {
    ...
    if (bannerImage) {
      if (user.bannerImage) {
        await cloudinary.uploader.destroy(
          user.bannerImage.split("/").pop().split(".")[0]
        );
      }
    ...
    if (userName) {
      ...
      if (doesExist && doesExist._id.toString() !== userId.toString())
    ...
    return res
      .status(200)
      .json({ message: "user profile updated", data: user });
  } catch (error) {
...
export const getSuggestedUsers = async function (req, res) {
  try {
    ...
    const filteredUsers = users.filter(
      (u) => !usersFollowedByMe.following.includes(u._id.toString())
    );
